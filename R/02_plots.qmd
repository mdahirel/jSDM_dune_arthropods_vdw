...
A note on nomenclature: in the data and the model, the different biogeographic sectors were named 'districts', following Dutch nomenclature: https://nl.wikipedia.org/wiki/Floradistrict. Following reviewer comments, we rename them to "biogeographic sectors" here in the plots
...
relabeled_tree$tip.label <- paste0("*", relabeled_tree$tip.label, "*") |>
  str_replace("_", " ")
...
p_V <- ggplot(varpart) +
  ...
  scale_fill_brewer("Component",
    ...
    breaks = c(
      ...
      "Local environmental variables", "Biogeographic sector",
      "Random effects (sample)"
    )
  ) +
...
p_VC <- (p_tree+ guides(fill="none") | p_V ) + plot_layout(widths = c(0.2, 0.8),guides="collect")& theme(legend.position = "bottom")
```
...
p_beta_district <- ggplot(betas_district) +
  ...
  facet_grid(cols = vars(coef)) +
  coord_cartesian(xlim = c(-5, 2)) +
  ggtitle(label = "Biogeographic sector-level contrasts (vs. reference level: Flemish dunes)") +
  theme_bw() +
  theme(axis.text.y = element_markdown(), legend.position = "none")
```
...
## model performance metrics, part 1
...
```{r save-perfplot}
ggsave(filename = here("plots", "Splot_performance.pdf"), plot = p_perf, device = cairo_pdf, width = 7.5, height = 6)
```

## model performance metrics, part 2: logOR of AUC-PR vs random baseline

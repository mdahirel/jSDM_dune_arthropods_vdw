...
ggsave(filename = here("plots","Fig5_beta_district.pdf"), 
...
```{r save-beta-local}
ggsave(filename = here("plots","Fig6_beta_local.pdf"), 
...
ggsave(filename = here("plots","Fig7_gamma_district.pdf"), 
...
```{r save-gamma-local}
ggsave(filename = here("plots","Fig8_gamma_local.pdf"), 
       plot = p_gamma2, device = cairo_pdf, width = 11.5, height = 3.5)
```
...
p_logOR  <- ggplot(fitmetrics) +
  geom_segment(aes(y = species, x = qlogis(AUC.lower)-qlogis(occurrence_obs), 
                   ...
                   col = AUC.lower > occurrence_obs), linewidth = 3) +
  scale_colour_manual(values = c(`FALSE` = "lightgrey", `TRUE` = "#f1a340"))+

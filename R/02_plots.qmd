...
A note on nomenclature: in the data and the model, the different biogeographic areas are named 'districts', following Dutch nomenclature: https://nl.wikipedia.org/wiki/Floradistrict. Following a reviewer's comment, we rename them to "biogeographic areas" here in the plots
...
p_V <- ggplot(varpart) +
  ...
  scale_fill_brewer("Component",
    ...
    breaks = c(
      ...
      "Local environmental variables", "Biogeographical area",
      "Random effects (sample)"
    )
  ) +
...
p_beta_district <- ggplot(betas_district) +
  ...
  facet_grid(cols = vars(coef)) +
  coord_cartesian(xlim = c(-5, 2)) +
  ggtitle(label = "Biogeographical area-level contrasts (vs. reference level: Flemish dunes)") +
  theme_bw() +
  theme(axis.text.y = element_markdown(), legend.position = "none")
```
